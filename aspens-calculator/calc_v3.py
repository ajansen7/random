...
def process_aspens(image_path):
    ...
    cv2.createTrackbar("PINE Hue Min", "Color Tuner", PINE_HUE_CENTER - 35, 179, nothing)
    cv2.createTrackbar("PINE Hue Max", "Color Tuner", PINE_HUE_CENTER + 35, 179, nothing)
    cv2.createTrackbar("ASPEN Hue Min", "Color Tuner", ASPEN_HUE_CENTER - 20, 179, nothing)
    cv2.createTrackbar("ASPEN Hue Max", "Color Tuner", ASPEN_HUE_CENTER + 20, 179, nothing)
    ...
    while True:
        ...
        mask_pine = cv2.inRange(hsv_img, np.array([p_h_min, 60, 70]), np.array([p_h_max, 255, 255]))
        mask_aspen = cv2.inRange(hsv_img, np.array([a_h_min, 60, 70]), np.array([a_h_max, 255, 255]))
    ...
    for row in range(GRID_ROWS):
        for col in range(GRID_COLS):
            ...
            mean_color = cv2.mean(hsv_img, mask=mask_cell)
            h, s, v = mean_color[:3]
            
            if v < 70 or s < 60: continue
            
            is_pine = p_h_min <= h <= p_h_max
            is_aspen = a_h_min <= h <= a_h_max
            ...
            if is_pine and not is_aspen: grid_matrix[row][col] = 1
                ...
                pine_dist = min(abs(h - p_h_min), abs(h - p_h_max))
                aspen_dist = min(abs(h - a_h_min), abs(h - a_h_max))
                grid_matrix[row][col] = 1 if pine_dist < aspen_dist else 2
